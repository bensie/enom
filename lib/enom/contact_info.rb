module Enom
  module ContactInfo
    ...
    CONTACT_TYPES.each do |contact_type|
      ...
      # ...
      # end
      define_method "update_#{contact_type.downcase}_contact_info" do |data = {}|

        # Remove attributes that are not in Enom's list of available fields
        data.select!{|k| FIELDS.map{|f| f[:name] }.include?(k)}

        # Check to make sure all required fields are present
        FIELDS.each do |field|
          if field[:required]
            if data[field[:name]].nil? || data[field[:name]].empty?
              raise Error, "#{field[:name]} is required to update contact info"
            end
          end
        end

        # response = Client.request('Command' => 'Contacts', 'SLD' => sld, 'TLD' => tld)["interface_response"]["Contacts"]
        raise NotImplementedError
      end
